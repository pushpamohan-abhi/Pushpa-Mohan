...
import { getStateColor } from './DfaAnimatorWidget';
      ...
      <div className="overflow-x-auto rounded-xl border border-slate-700/40 shadow-inner">
        <table className="w-full text-left border-collapse">
          <thead>
            ...
            >
              <th className="py-4 px-5 font-black text-sm sm:text-base">
                {headers[0] || 'State (Q)'}
              </th>
              {headers.slice(1).map((h, idx) => (
                <th
                  key={idx}
                  className="py-4 px-5 text-center font-mono font-black text-base sm:text-lg border-l border-slate-700/30"
                >
                  <span
                    className={`inline-block px-3.5 py-1 rounded-lg ${
                      isLight ? 'bg-indigo-100 text-indigo-900 border border-indigo-300' : 'bg-cyan-950 text-cyan-100 border border-cyan-400'
                    }`}
                  >
                    {h}
                  </span>
          ...
          <tbody className={`divide-y text-sm ${isLight ? 'divide-slate-200' : 'divide-blue-900/40'}`}>
            {rows.map((row, rIdx) => {
              const stColor = getStateColor(row.state);
              return (
                <tr
                  key={rIdx}
                ...
                >
                  {/* State Name & Badges */}
                  <td className="py-4 px-5 font-mono font-bold text-base">
                    <div className="flex items-center gap-2.5 flex-wrap">
                      {row.isStart && (
                        <span
                          className="px-2.5 py-1 rounded-md text-xs font-black uppercase tracking-wider bg-blue-500/20 text-blue-300 border border-blue-400 flex items-center gap-1 shrink-0"
                      ...
                      {row.isAccept && (
                        <span
                          className="px-2.5 py-1 rounded-md text-xs font-black uppercase tracking-wider bg-emerald-500/20 text-emerald-300 border border-emerald-400 flex items-center gap-1 shrink-0"
                          title="Accepting / Final State (F)"
                        >
                          <Star className="w-3.5 h-3.5 text-emerald-400 fill-emerald-400/30" />
                          ACCEPT
                        </span>
                      )}

                      <span
                        className="px-3 py-1.5 rounded-xl text-base font-black border font-mono inline-block shadow-sm"
                        style={{
                          backgroundColor: isLight ? '#f1f5f9' : stColor.fill,
                          borderColor: stColor.stroke,
                          color: isLight ? stColor.fill : '#ffffff',
                        }}
                      >
                        {row.state}
                      </span>
                  ...
                  {row.cells.map((cell, cIdx) => (
                    <td
                      key={cIdx}
                      className="py-4 px-5 text-center font-mono font-bold border-l border-slate-700/30"
                    >
                      <span
                        className="inline-block px-4 py-1.5 rounded-xl text-base font-black border transition-transform hover:scale-105 shadow-md"
                        style={{
                          backgroundColor: isLight ? '#f8fafc' : stColor.box,
                          borderColor: stColor.stroke,
                          color: isLight ? stColor.fill : '#ffffff',
                        }}
                      >
